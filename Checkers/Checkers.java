		...
		if(this.pieceSelected.getName().equals("BLANK")){//Checks if the user has already selected a piece
				...
				|| (this.currentGameBoardState.get(move).getName().startsWith("b") && userToken == 2))//checks if the first piece the user selected is a valid piece
			{
				if(!jumpCheck || (jumpCheck && canJump(move))){//if there are possible jumps on the board check for that if not, this will still return true
					this.pieceSelected = new CheckersPiece(this.currentGameBoardState.get(move));//This shows that the user has officially selected a piece
					selection = move;//This saves the Pair<X, Y> object so the code can reference it later
					return true;//finally returns true although in our case we might want to return false maybe use a boolean variable called gameStateChanged
				}
			}
			else{
				return false; 
			...
			if(x-selection.getKey()==0 && y-selection.getValue()==0) //If we click the piece again it'll undo the selected checkers piece so we're free to choose another one
			{
				...
				return true;//Again, maybe user the gamestateChange variable here
			}
...
//				System.out.println("CHECK THIS4 " + this.currentGameBoardState.get(move).getName());
	...
	public void resetBoard() {
		...
		for (int i = 0; i < this.numColumns; i += 2){
			this.currentGameBoardState.get(new Pair<Integer, Integer>(7, i)).setName("r");
			...
			this.currentGameBoardState.get(new Pair<Integer, Integer>(6, i+1)).setName("r");
			...
			this.currentGameBoardState.get(new Pair<Integer, Integer>(5, i)).setName("r");
			...
			this.currentGameBoardState.get(new Pair<Integer, Integer>(0, i+1)).setName("b");
			...
			this.currentGameBoardState.get(new Pair<Integer, Integer>(1, i)).setName("b");
			...
			this.currentGameBoardState.get(new Pair<Integer, Integer>(2, i+1)).setName("b");
	...
	public void printBoard() {
		...
		for(int i = 0; i < this.numRows; i++){
			System.out.print(i + " ");
		    for(int j = 0; j < this.numColumns; j++){
		    	if(this.currentGameBoardState.get(new Pair<Integer, Integer>(i,j)).getName().length() > 1){
		    		System.out.print(this.currentGameBoardState.get(new Pair<Integer, Integer>(i,j)).getName().substring(0, 2) + "    ");
		    	}else{
		    		System.out.print(this.currentGameBoardState.get(new Pair<Integer, Integer>(i,j)).getName().substring(0, 1) + "     ");
		    	}
